    ...
    for chr in 8; do
        ...
        if [ "${input}" = "vcf" ]; then
            ...
            dx run final-group-tests \
                    ...
                    -i GENE_TRAIT_PAIRS_TO_TEST="/brava/duncan/gene_phenotype_pairs_with_conditioning_variants_${anc_upper}_${p_threshold}.tsv" \
                    -f json/file_paths_${anc}_chr${chr}.json \
                    -i ANC=${anc} \
                    -i CHR=${chr} \
                    --instance-type "mem2_ssd1_v2_x4" --priority high \
                    --destination /brava/duncan/outputs/final-group-tests-vcf/${anc_upper}_${p_threshold}/ -y \
                    --name "final-group-tests-vcf_${anc}_${chr}_${p_threshold}"
        else
            echo ${input}
            dx run final-group-tests \
                    ...
                    --destination /brava/duncan/outputs/final-group-tests-plink/${anc_upper}_${p_threshold}/ -y \
                    --name "final-group-tests-plink_${anc}_${chr}_${p_threshold}"
            ...
            #         -i SPARSE_MATRIX="/brava/inputs/GRM_autosomes/brava_${anc_upper}_relatednessCutoff_0.05_5000_randomMarkersUsed.sparseGRM.mtx" \
            #         -i SPARSE_MATRIX_IDs="/brava/inputs/GRM_autosomes/brava_${anc_upper}_relatednessCutoff_0.05_5000_randomMarkersUsed.sparseGRM.mtx.sampleIDs.txt" \
            #         -i GENE_TRAIT_PAIRS_TO_TEST="/brava/duncan/gene_phenotype_pairs_with_conditioning_variants_${anc_upper}_test.tsv" \
            #         -f json/file_paths_${anc}_chr${chr}.json \
            #         -i ANC=${anc} \
            #         -i CHR=${chr} \
            #         --instance-type "mem2_ssd2_v2_x4" --priority high \
